...
def seed_all_datasets(target_engine=None):
    """
    Seed datasets into the default database or a specified database engine.
    """

    # Use the supplied engine during testing.
    ...
    if df_math is not None:
        ...
        print(
            f"[SUCCESS] Ingested {len(df_math)} rows "
            "(Math Performance)"
        )
    ...
    if df_port is not None:
        ...
        print(
            f"[SUCCESS] Ingested {len(df_port)} rows "
    ...
    if df_dropout is not None:
        ...
        print(
            f"[SUCCESS] Ingested {len(df_dropout)} rows "
            "(Dropout Risk Records)"
        )

    # 3. Online Learning Engagement & AI CRM Scoring
    df_eng = load_csv(
        "data/cleaned/04_online_engagement/"
        "online_learning_engagement.csv"
    )
    ...
    if df_eng is not None:
        ...
        print(
            f"[SUCCESS] Ingested {len(df_eng)} rows "
            "(Scored Student Leads)"
        )

    # 4. Education Marketing Datasets
    df_meta = load_csv(
        "data/cleaned/05_education_marketing/"
        "marketing_campaignmeta.csv"
    )

    if df_meta is not None:

        df_meta.to_sql(
            "marketing_campaign_meta",
            con=db_engine,
            if_exists="append",
            index=False
        )
        ...
        print(
            f"[SUCCESS] Ingested {len(df_meta)} rows "
            "(Marketing Metadata)"
        )

    df_perf = load_csv(
        "data/cleaned/05_education_marketing/"
        "marketing_campaignperformance.csv"
    )
    ...
    if df_perf is not None:
        ...
        print(
            f"[SUCCESS] Ingested {len(df_perf)} rows "
            "(Marketing Campaign Performance)"
        )

    df_rates = load_csv(
        "data/cleaned/05_education_marketing/"
        "marketing_channelrates.csv"
    )

    if df_rates is not None:

        df_rates.to_sql(
            "marketing_channel_rates",
            con=db_engine,
            if_exists="append",
            index=False
        )
        ...
        print(
            f"[SUCCESS] Ingested {len(df_rates)} rows "
            "(Channel Ad Rates)"
        )
